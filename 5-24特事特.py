# -*- coding: utf-8 -*-
"""
Created on Thu May 24 11:08:19 2018
综合测试
@author: Windows
"""
import cv2  
...
img1 = cv2.merge([b8 & bin_img, g8 & bin_img, r8 & bin_img]) 
win = cv2.namedWindow('Result2', flags=0)
cv2.imshow('Result2', img1)
src = cv2.imread('007-b.jpg') 
fsrc = np.array(src, dtype=np.float32) / 255.0  
(b,g,r) = cv2.split(fsrc)  
gray = 2*r  - b - g  
(minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray)  
...
(thresh, bin_img) = cv2.threshold(gray_u8, -1.0, 255, cv2.THRESH_OTSU)  
(b8, g8, r8) = cv2.split(src)  
img2 = cv2.merge([b8 & bin_img, g8 & bin_img, r8 & bin_img]) 
win = cv2.namedWindow('Result3', flags=0)
cv2.imshow('Result3', img2)  
...
#print (X1)
    ...
    maxY1 = max(Y1)
    minY1 = min(Y1)
    cv2.rectangle(img1, (minX, minY), (maxX, maxY), (0, 0, 255), 20)
    cv2.rectangle(img2, (minX1, minY1), (maxX1, maxY1), (0, 0, 255), 20)

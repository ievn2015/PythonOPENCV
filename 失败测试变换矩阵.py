...
o_pts = np.float32([ kpts1[m.queryIdx].pt for m in alltest ]).reshape(-1,1,2) #获取匹配到的坐标存在np中第一张图
d_pts = np.float32([ kpts2[m.trainIdx].pt for m in alltest ]).reshape(-1,1,2)
M, mask = cv2.findHomography(o_pts, d_pts, cv2.RANSAC, 5.0)
print (M)
dst = cv2.perspectiveTransform(o_pts,M)
print (o_pts,dst)

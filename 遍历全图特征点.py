...
if len(good) > MIN_MATCH_COUNT:
 ...
 #   dst = cv2.perspectiveTransform(pts,M)
    # 绘制边框
    cv2.polylines(img1,[np.int32(src_sub_copy)],True,(0,0,255),3, cv2.LINE_AA)
    cv2.polylines(canvas,[np.int32(dst_sub_copy)],True,(0,0,255),3, cv2.LINE_AA) # 括号内颜色，20越大越粗
    #画框需要把浮点型化成整形
    # (8) 画线
    matched = cv2.drawMatches(img1,kpts1,canvas,kpts2,good,None,(0,255,0))#括号内为连线颜色，不写则颜色随机,可以在后面加特征点颜色，不写就颜色随机)

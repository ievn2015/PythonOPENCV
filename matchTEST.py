...
good = [m1 for (m1, m2) in matches if m1.distance < 1 * m2.distance]
...
if len(good) > MIN_MATCH_COUNT:
    ...
    M, mask = cv2.findHomography(src_pts, src_pts, cv2.RANSAC,5.0)
...
## (8) drawMatches
matched = cv2.drawMatches(img1,kpts1,canvas,kpts2,good,(0,255,0))#,**draw_params)
